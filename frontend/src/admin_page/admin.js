...
function Admin({ handleLogout }) {
  ...
  const fetchQueries = async () => {
    const response = await fetch('http://localhost:8000/api/queries/'); // Adjust according to your Django server
  ...
  const addQuestion = async () => {
    if (newQuestion.trim() !== '' && newAnswer.trim() !== '') {
      ...
      await fetch('http://localhost:8000/api/queries/add/', {
        method: 'POST',
        headers: {
  ...
  const deleteQuery = async (queryId) => {
    const response = await fetch(`http://localhost:8000/api/queries/delete/${queryId}/`, {
      method: 'DELETE',
    });
  ...
  const updateQuestion = async () => {
    if (editingQueryId) { // Check if editingQueryId is valid
      if (newQuestion.trim() !== '' && newAnswer.trim() !== '') {
        ...
        const response = await fetch(`http://localhost:8000/api/queries/edit/${editingQueryId}/`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
